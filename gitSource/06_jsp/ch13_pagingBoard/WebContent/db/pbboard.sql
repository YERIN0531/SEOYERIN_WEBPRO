...
-- 8. 페이징을 위한 top_N구문 (startRow ~ endRow 까지 출력할 top-N)
SELECT * FROM BOARD ORDER BY REF DESC;
SELECT * FROM BOARD ORDER BY REF DESC, RE_STEP; -- REF는 큰거부터, RE_STEP은 작은것부터 (답변글로직)
                                                -- 1단계
                                                
SELECT ROWNUM RN, A.* FROM (SELECT * FROM BOARD ORDER BY REF DESC, RE_STEP) A; -- 2단계 
